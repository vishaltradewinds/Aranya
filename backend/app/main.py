...
def create_lot(payload:ProduceIn):
    with SessionLocal() as db:
        ...
        db.add(r); audit(db,"LOT_CREATED",lot_id,payload.producer_id,{"status":r.status,"regulatory_status":r.regulatory_status}); db.commit(); db.refresh(r); return to_lot(r)
...
def transition_lot(lot_id:str,payload:TransitionIn):
    with SessionLocal() as db:
        r=db.get(LotRecord,lot_id)
        if not r: raise HTTPException(404,"Lot not found")
        ...
        db.add(LotTransitionRecord(id=str(uuid4()),lot_id=lot_id,from_status=previous,to_status=target.value,actor=payload.actor,reason=payload.reason,occurred_at=datetime.now(timezone.utc)))
        audit(db,"LOT_STATUS_CHANGED",lot_id,payload.actor,{"from":previous,"to":target.value,"regulatory_status":d.status.value,"rule_ids":d.rule_ids,"reason":payload.reason})
